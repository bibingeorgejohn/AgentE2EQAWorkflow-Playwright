...
## Workflow Overview
This prompt workflow guides you through a complete 7-step QA process using Model Context Protocol (MCP) servers and AI agents to progress from a user story to committed, stable automated test scripts[cite: 1].
...
### Prompt
```text
I need to start a new testing workflow. Please read the user story from the file: user-stories/ecommerce-checkout.md
...
```text
Based on the user story ecommerce-checkout.md that we just reviewed, use the playwright-test-planner agent to:
...
* 🗂️ Organized test scenarios with a clear, hierarchical structure
...
* 🛠️ Manual test execution results


* 🖼️ Screenshots of the application at various states
...
* ✏️ Healed test scripts updated in `tests/saucedemo-checkout/`
...
Structure the report as: test-results/checkout-test-report.md
Include:
1. Executive Summary
...
* Target Context: `https://github.com/bibingeorgejohn/AgentE2EQAWorkflow-Playwright.git`
...
Git Repository URL: [https://github.com/bibingeorgejohn/AgentE2EQAWorkflow-Playwright.git](https://github.com/bibingeorgejohn/AgentE2EQAWorkflow-Playwright.git)
...
"feat(tests): Add complete test suite for checkout workflow
...
* 🏁 Workspace tracking initialized and sync completed


* 📦 Clean staging of all assets (`user-stories/`, `specs/`, `tests/`, `test-results/`)


* 🚀 Remote upstream push verification and commit hash output
